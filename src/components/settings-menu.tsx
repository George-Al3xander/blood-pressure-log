...
const menuItems: { [key: string]: (MenuItemProps & LinkProps)[] } = {
  logged: [
    ...
    {
      children: "btn_logout.default",
      href: "/api/mongo/users/logout",
      component: Link,
    },
  ],
  notLogged: [
...
const SettingsMenu = ({ loggedIn }: { loggedIn: boolean }) => {
  ...
  return (
    <div>
      ...
      >
        {menuItem.map(({ children, ...item }) => (
          <MenuItem onClick={handleClose} key={children?.toString()} {...item}>
