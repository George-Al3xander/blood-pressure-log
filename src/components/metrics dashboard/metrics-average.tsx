    ...
    >
      <Stack>
        <Stack sx={{ my: 1 }} alignItems={"flex-end"}>
          {checked ? (
            <CloseIcon onClick={handleChange} color="error" {...iconProps} />
          ) : (
            <InfoIcon onClick={handleChange} color="info" {...iconProps} />
          )}
        </Stack>
...
const MetricsAverage = (props: MetricsResponce) => {
  return (
    <Grid spacing={4} container>
      {keys.map((key) => (
        <Grid key={key} xs={12} md={6} item>
          <MetricCard path={key} value={props[key as "pulse_average"] as number} />
        </Grid>
      ))}
    </Grid>
  )
