...
import { CreateReportProps, Field, UpdateReportProps } from "@/types/types";
...
export default function ManageLogForm({
    ...
    const onSuccess = async (data: TReportData) => {
        ...
        if (res.success) {
            if (res.report) {
                ...
                toast.success(
                    tZod(
                        `reportAction.${type == "POST" ? "create" : "update"}.success`,
                    ),
                );
            }
            reset({
    ...
    const {
        ...
        defaultValues: {
            ...
            rating:
                defaultValue && defaultValue.rating ? defaultValue.rating : 3,
        },
    });
            ...
            <Grid spacing={2} container>
                ...
                <Grid item xs={12}>
                    ...
                    >
                        {isBusy
                            ? t(
                                  `btn_${type == "POST" ? "create" : "edit"}.process`,
                              )
                            : t(
                                  `btn_${type == "POST" ? "create" : "edit"}.default`,
                              )}
                    </Button>
                </Grid>
            </Grid>
