...
export async function middleware(request: NextRequest) {
    ...
    const handleI18nRouting =  createIntlMiddleware({locales,defaultLocale: request.headers.get('x-default-locale') as "uk" || defaultLocale});
   
    const homeUrl = new URL('/', request.url);
    const authUrl = new URL('/auth/login', request.url);

    const response = handleI18nRouting(request);
    response.headers.set('x-default-locale', request.headers.get('x-default-locale') as "uk" || defaultLocale)
