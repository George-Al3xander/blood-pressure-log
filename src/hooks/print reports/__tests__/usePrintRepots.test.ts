...
import { TPrintHook } from "@/types/types";
...
describe("Behavior", () => {
    describe("Initial render", () => {
        ...
        beforeEach(
            () => (printHook = renderHook(usePrintReports).result.current),
        );
        ...
        it("should return the right date range", () => {
            ...
            expect(dayjs(to).format("MM/DD/YYYY")).toBe(
    ...
    describe("After onValidationSuccess  call", () => {
        describe("reports all date", () => {
            ...
            beforeAll(async () => {
                ...
                await act(
                    async () =>
                        await printHook.result.current.onValidationSuccess({
                            from,
                            to,
                        }),
                );
            ...
            it("should change step to print", async () => {
                await waitFor(() =>
                    expect(printHook.result.current.step).toBe("print"),
                );
            });
        ...
        describe("error handle", () => {
            ...
            it("should change err status", async () => {
                ...
                await act(
                    async () =>
                        await result.current.onValidationSuccess({ from, to }),
                );
            ...
            it("should reset value to default after reset function", async () => {
                ...
                expect(
                    dayjs(result.current.dateRange.from).format("MM/DD/YYYY"),
                ).toBe(dayjs().subtract(1, "year").format("MM/DD/YYYY"));
                expect(
                    dayjs(result.current.dateRange.to).format("MM/DD/YYYY"),
                ).toBe(dayjs().format("MM/DD/YYYY"));
            });
        });
